...
header currentUser route = HH.nav
  ...
  [ HH.div
    ...
    , HH.ul
      ...
      , whenElem (isJust currentUser) $ navItem Greet
        [ HH.i [ css "ion-compose" ] [ HH.text " Greet" ] ]
      , whenElem (isNothing currentUser) $ navItem Login
          [ HH.text "Log in" ]
      , whenElem (isNothing currentUser) $ navItem Register
          [ HH.text "Sign up" ]
      ]
    ]
  ]
  where
    navItem r html = HH.li
      [ css "nav-item" ]
      [ HH.a
