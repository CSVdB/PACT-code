...
getCoachWorkoutInfoFromUUID :: MonadIO m => CoachWorkoutUUID -> SqlPersistT m (Maybe CoachWorkoutInfo)
getCoachWorkoutInfoFromUUID uuid = do
  workoutM <- entityVal <$$> getBy (UniqueCoachWorkout uuid)
  case workoutM of
    Nothing -> pure Nothing
    Just workout -> workoutToInfo workout
