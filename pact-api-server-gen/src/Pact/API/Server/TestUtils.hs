...
module Pact.API.Server.TestUtils where
    ...
    withSqlitePool ":memory:" 1 $
      \pool -> do
        ...
        liftIO $ do
          ...
          let serverApp = pactAPIServerApp serverEnv
...
withAnyNewUser cenv func =
  forAllValid $ \rf -> ioProperty $ withNewUser cenv rf func
...
testLogin cenv lf = do
  ...
  case sessionHeader of
    ...
    Header setCookieText -> do
      let cookies =
            parseSetCookie . TE.encodeUtf8 <$> T.lines setCookieText
