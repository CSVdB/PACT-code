...
in
{
        ...
        mkOption {
          type =
            types.submodule {
              options =
                {
                  enable = mkEnableOption "Pact API Server";
                  log-level =
                    mkOption {
                      type = types.str;
                      example = "Debug";
                  ...
                  local-backup =
                    mkOption {
                      type = types.nullOr (
                        types.submodule {
                          options = {
                            enable = mkEnableOption "Pact API Server Local Backup Service";
                            backup-dir = mkOption {
                              type = types.str;
                              example = "backup/api-server";
                              default = "backup/api-server";
                              description = "The directory to store backups in, relative to the /www/pact/${envname} directory or absolute";
                            };
                          };
                        }
                      );
  ...
  config =
    let
      ...
      api-server-database-file = api-server-working-dir + "pact-server-database.sqlite3";
      # The api server
      api-server-service =
        with cfg.api-server;
        optionalAttrs enable {
          "pact-api-server-${envname}" = {
            description = "Pact API Server ${envname} Service";
            wantedBy = [ "multi-user.target" ];
            environment =
              {
                "PACT_API_SERVER_LOG_LEVEL" =
                  "${builtins.toString log-level}";
                "PACT_API_SERVER_PORT" =
                  "${builtins.toString port}";
                "PACT_API_SERVER_DATABASE" = api-server-database-file;
              };
            script =
              ''
                mkdir -p "${api-server-working-dir}"
                cd ${api-server-working-dir};
                ${pactPackages.pact-api-server}/bin/pact-api-server
              '';
            serviceConfig =
              {
                Restart = "always";
      ...
      local-backup-service =
        optionalAttrs (cfg.api-server.enable or false) (
          optionalAttrs (cfg.api-server.local-backup.enable or false) (
            ...
            {
              "pact-api-server-local-backup-${envname}" = {
                description = "Backup pact-api-server database locally for ${envname}";
                wantedBy = [ ];
                script =
                  ''
                    mkdir -p ${backup-dir}
      ...
      local-backup-timer =
        optionalAttrs (cfg.api-server.enable or false) (
          optionalAttrs (cfg.api-server.local-backup.enable or false) (
            ...
            {
              "pact-api-server-local-backup-${envname}" = {
                description = "Backup pact-api-server database locally for ${envname} every twelve hours.";
